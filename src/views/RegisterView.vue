    ...
    <div class="w-full max-w-sm">
      ...
      >
        <div class="text-center mb-6">
          ...
          <h1 class="text-2xl font-bold text-app">{{ $t('auth.registerTitle') }}</h1>
          <p class="text-sm text-muted mt-1">{{ $t('auth.registerSubtitle') }}</p>
        </div>
        ...
        <label class="label">{{ $t('auth.fieldName') }}</label>
        <input v-model="name" type="text" required autocomplete="name" class="input mb-4" />
        ...
        <p class="text-center text-sm text-muted mt-4">
          {{ $t('auth.hasAccount') }}
          <router-link to="/login" class="text-brand-600 dark:text-brand-400 hover:underline font-medium">
            {{ $t('auth.signIn') }}
          </router-link>
        </p>
      </form>
